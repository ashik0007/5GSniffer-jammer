...
// Static member definitions for dataset extraction
std::shared_ptr<DatasetWriter> nr::pdcch::dataset_writer_ = nullptr;
HarqStateTracker nr::pdcch::harq_tracker_;
CarrierConfig nr::pdcch::carrier_config_;
bool nr::pdcch::dataset_initialized_ = false;

void nr::pdcch::init_dataset(const std::string& filepath) {
  if (!dataset_initialized_) {
    dataset_writer_ = std::make_shared<DatasetWriter>(filepath);
    dataset_initialized_ = true;
    SPDLOG_INFO("DCI dataset writer initialized: {}", filepath);
  }
}
  ...
  */
  ...
  int pdcch::decode_pdcch(symbol& symbol, std::vector<std::complex<float>>& pdcch_symbols, dci dci_, srsran_pdcch_nr_res_t* res, bool rep_opt, int64_t metadata, int symbol_in_chunk)
  {
    ...
    if (res->crc){
      ...
      int64_t unix_ts_ms = std::chrono::duration_cast<std::chrono::milliseconds>(
        std::chrono::system_clock::now().time_since_epoch()).count();
      SPDLOG_INFO("Found DCI PDCCH DCI: unix_ts_ms = {}, RNTI = {}, AL = {}, DCI size {}, Time = {}, Samples from start = {}, Slots from samples from start = {} Slot within frame = {}, Symbol within slot = {}, binary dci is {}, correlation is {}",
      unix_ts_ms, dci_.get_rnti(), dci_.get_found_aggregation_level(), dci_.get_nof_bits(), sample_time + symbol_in_chunk* 0.001, sample_time, symbol_in_chunk, symbol.slot_index, symbol.symbol_index, dci_string, dci_.get_correlation());

      // ── DCI Dataset Extraction ───────────────────────────────────────────
      if (!dataset_initialized_) {
        init_dataset();
      }
      if (dataset_writer_) {
        // Derive format from first payload bit: 0 = UL (0_1), 1 = DL (1_1)
        std::string fmt = (dci_payload[0] == 0) ? "0_1" : "1_1";
        {
          DCIFields fields = parse_dci(dci_payload.data(), dci_.get_nof_bits(),
                                       fmt, carrier_config_);
          bool is_dl = (fmt == "1_1");
          fields.is_retransmission = harq_tracker_.check_retransmission(
            dci_.get_rnti(), fields.harq_process_id, fields.ndi, is_dl);

          dataset_writer_->write_record(fields, dci_.get_rnti(),
            dci_.get_rnti_type(), sample_time + symbol_in_chunk * 0.001f,
            symbol.slot_index, symbol.symbol_index,
            dci_.get_found_aggregation_level(), dci_.get_correlation(),
            unix_ts_ms);
        }
      }
      // ── End DCI Dataset Extraction ───────────────────────────────────────
